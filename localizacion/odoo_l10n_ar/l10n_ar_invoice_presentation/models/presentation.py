...
class Presentation:
    ...
    def get_fecha(self, invoice):
        return self.helper.format_date(invoice.invoice_date)
    ...
    def get_puntoDeVenta(self, invoice):
        ...
        """
        return invoice.voucher_name.split('-')[0]
    ...
    def get_importeTotalNG_purchase(self, invoice):
        # Validacion de SIAP:
        # "EL CAMPO IMPORTE TOTAL DE CONCEPTOS QUE NO INTEGRAN EL PRECIO
        # NETO GRAVADO DEBE SER IGUAL A CERO CUANDO SON CPTES. B O C"
        if invoice.voucher_type_id.denomination_id in [self.data.type_b, self.data.type_c]:
            return str(0)
        else:
            return self.get_importeTotalNG(invoice)
    ...
    def get_cantidadAlicIva(self, invoice):
        ...
        cantidadAlicIva = [
            ...
            if tax.tax_line_id.is_vat
            and tax.tax_line_id != self.data.tax_purchase_ng
            and not tax.tax_line_id.is_exempt
        ]
    ...
    def get_codigoOperacion(self, invoice):
        ...
        len_total_taxes = len(invoice.line_ids.filtered(lambda x: x.tax_line_id))
        if not_taxed_taxes and (len_total_taxes - len(other_taxes)) == len(not_taxed_taxes):
            res = 'N'

        if exempt_taxes and (len_total_taxes - len(other_taxes)) == len(exempt_taxes):
            res = 'E'
...
class PurchasePresentation(Presentation):
    ...
    def get_invoice_notTaxed_taxes(self, invoice):
        return [
            tax for tax in invoice.line_ids
            if tax.tax_line_id == self.data.tax_purchase_ng
        ]
...
class SalePresentation(Presentation):
    ...
    def get_invoice_notTaxed_taxes(self, invoice):
        return [
            tax for tax in invoice.line_ids
            if tax.tax_line_id == self.data.tax_sale_ng
        ]
