...
class AccountMove(models.Model):
    ...
    def _recompute_dynamic_lines(self, recompute_all_taxes=False, recompute_tax_base_amount=False):
        ...
        # De esta manera evitamos que borre el move_line en 0 que genera la percepcion
        ...
        if not in_draft_mode:
            # Cuando se crea desde el back-end, la idea es re-calcular los montos debe-haber de todas las move lines
            # para que quede el asiento balanceado, entonces volvemos a ejecutarla pero sin re-calcular impuestos.
            super(AccountMove, self.with_context(check_move_validity=False))._recompute_dynamic_lines()
        return res
    ...
    def assign_perception_values(self):
        for perception in self.perception_ids:
            ...
            line_with_tax.with_context(check_move_validity=False).update(vals)
    ...
    def add_perceptions(self):
        ...
        self.invoice_line_ids.filtered(
            lambda x: x.product_id and x.product_id.perception_taxable or not x.product_id
        ).update({
