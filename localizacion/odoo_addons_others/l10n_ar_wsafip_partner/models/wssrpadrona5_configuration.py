...
class WsSrPadronA5Configuration(models.Model):

    _name = 'wssrpadrona5.configuration'
    ...
    def check_unique_ticket(self):
        for wsfe in self:
            if wsfe.search_count([('wsaa_token_id', '=', wsfe.wsaa_token_id.id)]) > 1:
                raise ValidationError('Ya existe una configuracion de padron asociado a ese token')
