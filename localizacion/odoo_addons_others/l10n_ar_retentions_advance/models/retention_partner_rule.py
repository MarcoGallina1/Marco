    ...
    def _check_repeat(self):
        for rule in self:
            rules = self.search([
                ...
                ('retention_id', '=', rule.retention_id.id),
                ('activity_id', '=', rule.activity_id.id),
                ('id', '!=', rule.id)
            ])
            if rules:
                raise ValidationError("Existe mas de una regla con retencion {} y actividad {}.".format(
                    rule.retention_id.name, rule.activity_id.name if rule.activity_id else "vacia")
                )
