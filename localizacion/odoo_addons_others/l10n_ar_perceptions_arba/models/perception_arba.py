...
class PerceptionArba(models.Model):
    ...
    def _get_sign(self, p):
        return '-' if p.move_id.type == 'out_refund' else '0'
    ...
    def generate_file(self):
        ...
        if missing_vats or invalid_doctypes or invalid_vats:
            ...
            self.filename = 'AR-{vat}-{period}{fortnight}-{presentation_type}{activity}-LOTE{lot}.TXT'.format(
                vat=self.company_id.vat,
                period=self.date_from.strftime('%Y%m'),
                fortnight=self.fortnight if not self.presentation_type else '',
