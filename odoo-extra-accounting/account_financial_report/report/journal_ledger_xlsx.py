...
class JournalLedgerXslx(models.AbstractModel):
    ...
    def _get_report_columns(self, report):
        columns = [
            {"header": _("Serial No"), "field": "serial_no", "width": 18},
    ...
    def _generate_moves_content(self, workbook, sheet_name, report, res_data, moves):
        ...
        is_new_serial = report.is_new_serial
        serial_no = report.serial_no if report.is_new_serial else 0
        for move in moves:
            if is_new_serial and len(move["report_move_lines"]) > 0:
                serial_no += 1
