...
class JournalLedgerReport(models.AbstractModel):
    ...
    #     Moves = []
    #     move_data = {}
    #     serial_no = wizard.serial_no
    #     for move in moves:
    #         move_data[move.id] = self._get_moves_data(move, serial_no)
    #         serial_no += 1
    #         Moves.append(move_data[move.id])
    #     return moves.ids, Moves, move_data
    ...
    def _get_report_values(self, docids, data):
        ...
        if wizard.is_new_serial and wizard.group_option == 'journal':
            serial_no = wizard.serial_no
            for d in data_final['Journal_Ledgers']:
                for r in d['report_moves']:
                    serial_no += 1
                    r['serial_no'] = serial_no
        if wizard.is_new_serial and wizard.group_option == 'none':
            serial_no = wizard.serial_no
            for m in data_final['Moves']:
                if len(m['report_move_lines']) > 0:
